...
module WithGradientDescent =
  ...
  let fit: Result<Weights, ErrorResult list> = Either.either {
    let! gdParameters = GradientDescentParameters.create GradientDescent.Stochastic 0.01 0.01 1500u
    let! trainingParameters = TrainingParameters.create H Y
    let linearRegressionWithSGD = fitWithGradientDescent Regularization.Without gdParameters
    let fit = linearRegressionWithSGD trainingParameters
    return fit
  }
